...
def _build_system_prompt(cfg, knowledge_chunks):
    ...
    return (
        ...
        f"فیلدها را از همان متن استخراج کن (مثلاً «۲۰ تن میلگرد ۱۴ اصفهان» ⇒ product=میلگرد, size=14, factory=اصفهان, quantity=20 تن)، سپس محصول و قیمت را نشان بده. "
        f"وقتی به محصول لینک می‌دهی، فقط مسیر نسبی مثل /products/12 را بنویس؛ هیچ دامنه یا http یا example.com اضافه نکن."
        f"{handoff}\n"
        f"تاریخ امروز: {today}"
    )
