...
- Keep runtime media out of source deployment archives and prepare writable blog-media directories for the backend service during deployment.
- Return a structured service-unavailable response when media storage cannot be written instead of exposing an HTML 500 response.
...
- Affected operations: backend archive attributes and `ops/prepare_media.sh`, invoked by the manual deployment updater
