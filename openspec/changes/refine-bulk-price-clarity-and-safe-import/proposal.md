...
- Backend (tg1): `products/serializers.py` (+2 summary fields), `products/api_views.py` (prefetch), `products/views.py` (`admin_import_template`), `products/admin_import.py` (`upsert_product_row` tier + delivery blank-safe). No migration.
- Frontend (kavehmetal): `app/components/admin/BulkPricePanel.js` (+2 columns).
