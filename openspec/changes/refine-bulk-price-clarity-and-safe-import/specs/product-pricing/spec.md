...
When an admin explicitly submits a blank/null price from the product form or inline bulk-price table, the system SHALL remove the product's pricing tiers, mark the product as price inquiry, and leave all non-price product data unchanged. This explicit blank-price behavior is distinct from an omitted `price` field, which SHALL leave existing prices untouched.
...
#### Scenario: Clear inline price for inquiry
- **WHEN** an admin clears a product price in the product form or inline bulk-price table and saves it
- **THEN** the product has no active pricing tiers, its availability status is `inquiry`, and its name, specifications, and delivery details remain unchanged
