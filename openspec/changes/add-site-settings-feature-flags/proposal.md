...
- Frontend: a `SettingsProvider` that fetches settings; replace hard-coded "کاوه متال" with the dynamic `site_name`; gate the five sections (hide entry points + guard routes) by their flags; add an admin **Settings** panel to edit the name and toggle sections.
...
- Affected code (frontend, kavehmetal): new `lib/siteSettingsApi.js`, `components/SettingsProvider.js`, `components/SectionDisabled.js`, admin `SettingsPanel`; edits to `app/layout.js`, nav/menus/homepage, gated route pages, and the ~21 files containing "کاوه متال".
