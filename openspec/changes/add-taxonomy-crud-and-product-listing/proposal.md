...
The product taxonomy admin panel now exists, but admins need full lifecycle control over categories and controlled options. Public product lists also need to behave like steel price lists: backend-driven tabs, factory grouping, clear dimensions/origin, and correct price/inquiry/order actions.
...
- Finalize `/products` public list behavior around active category/type tabs, factory sections, row titles, origin display, price/inquiry/order actions, and pagination.
...
- Frontend: `/admin/dashboard` taxonomy panel gains edit and lifecycle controls; `/products` gains tighter factory-grouped price-list behavior.
