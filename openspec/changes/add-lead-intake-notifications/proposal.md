...
- **Event wiring:** registration (`accounts` RegisterAPIView + Google auth) calls `on_user_signup`; a `StoreOrder` pre/post-save signal fires `on_order_submitted` on the transition into `SUBMITTED`; the assistant `capture_lead`/`register_inquiry` tools call `on_chat_lead`.
